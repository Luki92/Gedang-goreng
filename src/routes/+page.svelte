<script>
    ...
    import { detectUniversal } from '$lib/utils/persona-detect.js';
    ...
    import AdminPersona from '$lib/components/admin/AdminPersona.svelte';
    ...
    if (typeof window !== 'undefined') {
        ...
        windowManager.register('admin-persona', AdminPersona);
    ...
    function handleGlobalKeydown(e) {
        if (e.ctrlKey && e.shiftKey && (e.key === 'L' || e.key === 'l')) {
            e.preventDefault();
            windowManager.toggle('terminal', { isTiled: false, width: 850, height: 550, originType: 'bc', title: 'TERMINAL' });
        }
    }
    ...
    onMount(() => {
        ...
        const unsubMusic = musicState.subscribe(s => {
            ...
            else { clearInterval(lyricInterval); lyricInterval = null; }
        });
        return () => {
    ...
    async function runAnimationLoop() {
        ...
        while (true) {
            ...
            await typingPromise;
            if (c1) { c1.classList.remove('cursor-active'); c1.classList.add('cursor-hidden'); }
            if (Math.random() < 0.4) {
                titleEl.innerHTML = 'TS<span class="split-cursor"></span>';
                const eggCursor = titleEl.querySelector('.split-cursor');
                const eggText = " PMO ICL🥀";
                for(let char of eggText) { eggCursor.insertAdjacentText('beforebegin', char); await sleep(50); }
                await sleep(300);
            ...
            titleEl.innerHTML = `<div class="ts-wrapper" id="ts-container"><span id="ts-left" class="flex items-baseline">T<span class="split-cursor"></span></span><span id="ts-right" class="flex items-baseline">S<span class="split-cursor"></span></span></div>`;
            await sleep(0);
            ...
            if (c1) { c1.classList.remove('cursor-hidden'); c1.classList.add('cursor-active'); }
            wTextNode = welcomeContainer.childNodes[0];
